...
(** External calls must be deterministic. *)
  ec_determ:
...
Lemma ec_symbol_preserved_iff
  sem sg
  (EP: extcall_properties sem sg)
  ge1 ge2
  (SEQUIV: Senv.equiv ge1 ge2)
  vargs m1
  :
  forall t vres m2,
    sem ge1 vargs m1 t vres m2 <-> sem ge2 vargs m1 t vres m2.
Proof.
  ii. split.
  - eapply ec_symbols_preserved; eauto.
  - eapply ec_symbols_preserved; eauto. symmetry. eauto.
Qed.

(** * Generalized Semantics of external functions *)

Definition extcall_sem_gen : Type :=
  Senv.t -> list val -> mem -> (trace -> val -> mem -> Prop) -> Prop.

Definition self_inj (f:meminj) : Prop :=
  forall b b' delta, f b = Some (b', delta) -> b = b' /\ delta = 0.

Record extcall_properties_gen (sem: extcall_sem_gen) (sg: signature) : Prop :=
  mk_extcall_properties_gen {
  
(** The return value of an external call must agree with its signature. *)
  ec_well_typed_gen
    ge vargs m1 S t vres m2
    (DSTEP: sem ge vargs m1 S)
    (ASTEP: S t vres m2)
    :
    <<RTTYPE: Val.has_rettype vres sg.(sig_res)>>;

(** The semantics is invariant under change of global environment that preserves symbols. *)
  ec_symbols_preserved_gen
    ge1 ge2 vargs m1 S
    (SENVEQ: Senv.equiv ge1 ge2)
    (DSTEPSRC: sem ge1 vargs m1 S)
    :
    exists S', <<DSTEPTGT: sem ge2 vargs m1 S'>> /\ <<USB: forall t v m', S t v m' <-> S' t v m'>>;

(** External calls cannot invalidate memory blocks.  (Remember that
  freeing a block does not invalidate its block identifier.) *)
  ec_valid_block_gen
    ge vargs m1 S t vres m2 b
    (DSTEP1: sem ge vargs m1 S)
    (ASTEP1: S t vres m2)
    (VLD1: Mem.valid_block m1 b)
    :
    <<VLD2: Mem.valid_block m2 b>>;

(** External calls cannot increase the max permissions of a valid block.
    They can decrease the max permissions, e.g. by freeing. *)
  ec_max_perm_gen
    ge vargs m1 S t vres m2 b ofs p
    (DSTEP: sem ge vargs m1 S)
    (ASTEP: S t vres m2)
    (VLD: Mem.valid_block m1 b)
    (PERM2: Mem.perm m2 b ofs Max p)
    :
    <<PERM1: Mem.perm m1 b ofs Max p>>;

(** External call cannot modify memory unless they have [Max, Writable]
   permissions. *)
  ec_readonly_gen
    ge vargs m1 S t vres m2 b ofs n bytes
    (DSTEP: sem ge vargs m1 S)
    (ASTEP: S t vres m2)
    (VLD1: Mem.valid_block m1 b)
    (LOAD1: Mem.loadbytes m2 b ofs n = Some bytes)
    (READONLY: (forall i, ofs <= i < ofs + n -> ~Mem.perm m1 b i Max Writable))
    :
    <<LOAD2: Mem.loadbytes m1 b ofs n = Some bytes>>;

(** External calls must commute with memory extensions, in the
  following sense. *)
  ec_mem_extends_gen
    ge vargs vargs' m1 m1'
    (MEM: Mem.extends m1 m1')
    (ARGS: Val.lessdef_list vargs vargs')
    S_tgt
    (DCALLTGT: sem ge vargs' m1' S_tgt)
  :
    (exists S_src (DCALLSRC: sem ge vargs m1 S_src),
       (forall t vres m2 (ACALLSRC: S_src t vres m2),
          (exists vres' m2' (ACALLTGT: S_tgt t vres' m2'),
...
(** External calls must commute with memory injections,
  in the following sense. *)
  ec_mem_inject_gen
    ge1 ge2 vargs vargs' m1 m1' f
    (SYMB: symbols_inject f ge1 ge2)
    (MEM: Mem.inject f m1 m1')
    (ARGS: Val.inject_list f vargs vargs')
    S_tgt
    (DCALLTGT: sem ge2 vargs' m1' S_tgt)
  :
    (exists S_src (DCALLSRC: sem ge1 vargs m1 S_src),
       (forall t vres m2 (ACALLSRC: S_src t vres m2),
          (exists vres' m2' (ACALLTGT: S_tgt t vres' m2'),
           exists f',
  ...
  (*   (ARGS: Val.inject_list f vargs vargs) *)
  (*   S *)
  (*   (DCALL: sem ge vargs m1 S) *)
  (* : *)
  (*   (forall t vres m2 (ACALL: S t vres m2), *)
  (*      (exists f', *)
  (*        <<RETV: Val.inject f' vres vres>> *)
  (*      /\ <<MEM: Mem.inject f' m2 m2>> *)
  (*      /\ <<MEMPRIVSRC: Mem.unchanged_on (loc_unmapped f) m1 m2>> *)
...
(* (** External calls produce at most one event. *) *)
(*   ec_trace_length: *)
(*     forall ge vargs m S t vres m', *)
(*     sem ge vargs m S -> S t vres m' -> (length t <= 1)%nat; *)
}.

(* existed extcall prop -> lifed prop global lemma needed *)

Definition extcall_singleton (S: trace -> val -> mem -> Prop) :=
  forall tr1 v1 m1, S tr1 v1 m1 -> forall tr2 v2 m2, S tr2 v2 m2 -> tr1 = tr2 /\ v1 = v2 /\ m1 = m2.

Section TOGEN.

Variable ec: extcall_sem.

Inductive ec_to_gen (se: Senv.t) (vl: list val) (m: mem) : (trace -> val -> mem -> Prop) -> Prop :=
| ec_gen_intros:
  ec_to_gen se vl m (fun t v m' => ec se vl m t v m').

(* Hypothesis ec_gen_same_set: *)
(*   forall se vl m S, ec_gen se vl m S <-> S = (fun t v m' => ec se vl m t v m'). *)

(* Hypothesis ec_simple_lift1: *)
(*   forall se vl m S, ec_lifted se vl m S -> S = (fun t v m' => ec se vl m t v m'). *)

(* Hypothesis ec_simple_lift2: *)
(*   forall se vl m S, S = (fun t v m' => ec se vl m t v m') -> ec_lifted se vl m S. *)

Lemma ec_implies_ec_gen
  sg
  (EC: extcall_properties ec sg)
  :
  <<ECLIFTED: extcall_properties_gen ec_to_gen sg>>.
Proof.
  econs; ii.
  - inv DSTEP. eapply ec_well_typed; eauto.
  - assert (exists S', ec_to_gen ge2 vargs m1 S' /\ (forall t v m, S t v m <-> S' t v m)).
    { inv DSTEPSRC. ss. exists (fun t v m => ec ge2 vargs m1 t v m). esplits.
      - econs; eauto.
      - ii. split.
        + eapply ec_symbols_preserved; eauto.
        + symmetry in SENVEQ. eapply ec_symbols_preserved; eauto. }
    des. exists S'. esplits; eauto.
  - inv DSTEP1. eapply ec_valid_block; eauto.
  - inv DSTEP. eapply ec_max_perm; eauto.
  - inv DSTEP. eapply ec_readonly; eauto.
  - exists (fun tr vres1 m2 => ec ge vargs m1 tr vres1 m2).
    esplit.
    { econs. }
    ii. exploit ec_mem_extends; eauto. ii. des.
    esplits; eauto. inv DCALLTGT. eauto.
  - exists (fun tr vres1 m2 => ec ge1 vargs m1 tr vres1 m2).
    esplits.
    { econs. }
    ii. exploit ec_mem_inject; eauto. ii. des.
    esplits; eauto. inv DCALLTGT. eauto.
  (* - inv DCALL. *)
  (*   exploit ec_mem_inject; eauto. ii. des. *)
  (*   exploit (ec_determ EC). *)
  (*   { eapply ACALL. } *)
  (*   { eapply H. } *)
  (*   i. des; subst. esplits; eauto. *)
Qed.

Lemma ec_to_gen_singleton
  sg
  (EC: extcall_properties ec sg)
  se vl m S
  (TOGEN: ec_to_gen se vl m S)
  :
  <<SINGLE: extcall_singleton S>>.
Proof. inv TOGEN. rr. ii. eapply ec_determ; eauto. Qed.

Lemma ec_to_gen_mem_extends
  sg
  (EC: extcall_properties ec sg)        
  ge vargs vargs' m1 m1'
  (MEM: Mem.extends m1 m1')
  (ARGS: Val.lessdef_list vargs vargs')    
  S_src
  (DSRC: ec_to_gen ge vargs m1 S_src)
  tr vres m2
  (ASRC: S_src tr vres m2)
  :
  exists S_tgt, exists vres', exists m2',
    <<DTGT: ec_to_gen ge vargs' m1' S_tgt>>
  /\ <<ATGT: S_tgt tr vres' m2'>>
  /\ <<RETV: Val.lessdef vres vres'>>
  /\ <<MEM: Mem.extends m2 m2'>>
  /\ <<PRIV: Mem.unchanged_on (loc_out_of_bounds m1) m1' m2'>>
.
Proof.
  inv DSRC. exploit ec_mem_extends; eauto.
  i. des. esplits; eauto. econs; eauto.
Qed.

Lemma ec_to_gen_mem_inject
  sg
  (EC: extcall_properties ec sg)        
  ge1 ge2 vargs vargs' m1 m1' f
  (SYMB: symbols_inject f ge1 ge2)
  ...
  (ARGS: Val.inject_list f vargs vargs')
  S_src
  (DSRC: ec_to_gen ge1 vargs m1 S_src)
  tr vres m2
  (ASRC: S_src tr vres m2)
  :
  exists f', exists S_tgt, exists vres', exists m2',
    <<DTGT: ec_to_gen ge2 vargs' m1' S_tgt>>
  /\ <<ATGT: S_tgt tr vres' m2'>>
  /\ <<RETV: Val.inject f' vres vres'>>
  /\ <<MEM: Mem.inject f' m2 m2'>>
  /\ <<MEMPRIVSRC: Mem.unchanged_on (loc_unmapped f) m1 m2>>
...
.
Proof.
  inv DSRC. exploit ec_mem_inject; eauto.
  i. des. esplits; eauto. econs; eauto.
Qed.
...
Lemma volatile_load_extends_backward
  ...
  :
  (exists v, <<VLOADSRC: volatile_load ge chunk m b ofs t v>> /\
        <<VLD: Val.lessdef v v'>>) \/
...
Lemma volatile_load_extends_fail
  ge chunk m b ofs m'
...
- (* volatile load *)
  ...
  (* make Senv.find_symbol ge1 id = Some b *)
  2: { right. ii. inv H3; clarify.
       exploit B; eauto. i. des. subst.
       exploit Senv.find_symbol_injective.
       { eapply H8. }
       { eapply H0. }
       i. des. subst. clarify. }
  ...
  { inv H1.
    ...
    - assert (Senv.public_symbol ge1 id0 = true).
      ...
      exists (Vptr b1 ofs0). split.
      { econs; eauto. }
      { econs; eauto. rewrite Ptrofs.add_zero; eauto. }
  }
  des.
  exploit B; eauto. i. des; subst.
  ...
  apply Val.load_result_inject. auto.
- (* normal load *)
  exploit Mem.loadv_inject_backward; eauto. simpl; eauto. simpl; intros [[v2 [X Y]] | FAIL].
...
(* after push capture, lift this step to two step *)
(* 1) int -> ptr & 2) vload  *)
Inductive volatile_load_sem_gen (chunk: memory_chunk) (ge: Senv.t):
              list val -> mem -> (trace -> val -> mem -> Prop) -> Prop :=
  | volatile_load_sem_gen_intro
      vl m t v (S: trace -> val -> mem -> Prop)
      (VLOAD: volatile_load_sem chunk ge vl m t v m)
      (IN: S t v m)
      (SINGLE: forall t1 v1 m1, S t1 v1 m1 -> t1 = t /\ v1 = v /\ m1 = m)
    :
      volatile_load_sem_gen chunk ge vl m S
| volatile_load_sem_gen_fail
      vl m
      (VLFAIL: forall t v, ~ volatile_load_sem chunk ge vl m t v m)
    :
      volatile_load_sem_gen chunk ge vl m (fun t v m => False)
.

Lemma volatile_load_ok:
  forall chunk,
  extcall_properties_gen (volatile_load_sem_gen chunk)
...
- r. inv DSTEP.
  2 :{ clarify. }
  eapply SINGLE in ASTEP. des; subst.
  inv VLOAD. inv H. eapply Val.load_result_rettype.
  eapply Mem.load_rettype; eauto.
(* symbols *) 
- inv DSTEPSRC.
  2: { esplits.
       - econs 2. ii. eapply VLFAIL.
         instantiate (1:=v). instantiate (1:=t).
         inv H. econs. 
         eapply volatile_load_preserved; eauto. symmetry. eauto.
       - ii. clarify. }
  inv VLOAD.
  esplits.
  + econs; eauto.
    econs. eapply volatile_load_preserved; eauto.
  + ii. eauto.
(* valid blocks *)
- inv DSTEP1; clarify. eapply SINGLE in ASTEP1. des; subst. eauto.
(* max perms *)
- inv DSTEP; clarify. eapply SINGLE in ASTEP. des; subst. eauto.
(* readonly *)
- inv DSTEP; clarify. eapply SINGLE in ASTEP. des; subst. eauto.
(* mem extends *) (* after capture -> maybe become two step *)
- inv DCALLTGT.
  2 :{ exists (fun t v m => False). esplits.
       - econs 2; eauto. ii. inv H.
         exploit volatile_load_extends; eauto. i. des.
         specialize (VLFAIL t v'). eapply VLFAIL.
         inv ARGS. inv H4. inv H7. econs; eauto.
       - clarify. }
  inv VLOAD. inv ARGS. inv H5. inv H3.
  2 :{ exists (fun t v m => False). esplits.
       - econs 2; eauto. ii. inv H0.
       - clarify. }
  exploit volatile_load_extends_backward; eauto. i. des.
  (* SRC volatile load fail *)
  2 :{ exists (fun t v m => False). esplits; clarify.
       econs 2; eauto. ii. inv H0. eapply VLOADFAIL in H8. clarify. }
  (* SRC volatile load success *)
  exists (fun tr v m => tr = t /\ v = v0 /\ m = m1). esplits; eauto.
  { econs; eauto. econs; eauto. }
  ii. des; subst.
  esplits; eauto. econs; eauto. eapply Ple_refl.
(* mem injects *) (* after capture -> maybe become two step *)
- inv DCALLTGT.
  2 :{ exists (fun t v m => False). esplits; clarify.
       econs 2; eauto. ii. inv H.
       inv ARGS. inv H4. inv H2.
  ...
  2 :{ exists (fun t v m => False). esplits; clarify.
       econs 2; eauto. ii. inv H0. }
  exploit volatile_load_inject_backward; eauto. i. des.
  (* SRC volatile load fail *)
  2 :{ exists (fun t v m => False). esplits; clarify.
       econs 2; eauto. ii. inv H0. eapply VLOADFAIL in H9. clarify. }
  (* SRC volatile load success *)
  exists (fun tr v m => tr = t /\ v = v0 /\ m = m1). esplits; eauto.
  { econs; eauto. econs; eauto. }
  ii. des; subst.
...
(* ec sound *)
(* - inv DCALL; clarify. *)
(*   inv VLOAD. inv ARGS. inv H6. *)
(*   exploit volatile_load_inject; eauto. i. des. *)
(*   eapply SINGLE in ACALL. des; subst. *)
(*   exists f. *)
(*   esplits; eauto. *)
(*   + inv H1; try (by econs). *)
(*     exploit SELF; eauto. i. des; subst. *)
(*     econs; eauto. rewrite Ptrofs.add_zero. auto. *)
(*   + eapply Mem.unchanged_on_refl. *)
(*   + eapply Mem.unchanged_on_refl. *)
(*   + red; intros. congruence. *)
Qed.
...
(* Definition volatile_store_sem_gen (chunk: memory_chunk) (ge: Senv.t): *)
(*   list val -> mem -> (trace -> val -> mem -> Prop) -> Prop := *)
(*   ec_to_gen (volatile_store_sem chunk). *)

(* Inductive volatile_store_sem_lift (chunk: memory_chunk) (ge: Senv.t): *)
(*               list val -> mem -> (trace -> val -> mem -> Prop) -> Prop := *)
(*   | volatile_store_sem_lift_intro *)
(*       vl m S *)
(*       (VSTORE: S = fun t v m' => volatile_store_sem chunk ge vl m t v m') *)
(*     : *)
(*       volatile_store_sem_lift chunk ge vl m S *)
(* . *)

(* Lemma volatile_store_lift_same_set: *)
(*   forall chunk se vl m S, volatile_store_sem_lift chunk se vl m S <-> S = (fun t v m' => volatile_store_sem chunk se vl m t v m'). *)
(* Proof. *)
(*   ii. split; ii. *)
(*   - inv H; eauto. *)
(*   - subst; econs; eauto. *)
...
Lemma volatile_store_ok_gen:
  forall chunk,
  extcall_properties_gen (ec_to_gen (volatile_store_sem chunk))
                         (mksignature (Tptr :: type_of_chunk chunk :: nil) Tvoid cc_default).
Proof.
  ii. eapply ec_implies_ec_gen. eapply volatile_store_ok.
Qed.
...
Lemma extcall_malloc_ok_gen:
   extcall_properties_gen (ec_to_gen extcall_malloc_sem)
                          (mksignature (Tptr :: nil) Tptr cc_default).
Proof.
  ii. eapply ec_implies_ec_gen. eapply extcall_malloc_ok.
Qed.
...
Lemma extcall_free_ok_gen:
   extcall_properties_gen (ec_to_gen extcall_free_sem)
                          (mksignature (Tptr :: nil) Tvoid cc_default).
Proof.
  ii. eapply ec_implies_ec_gen. eapply extcall_free_ok.
Qed.
...
Lemma extcall_memcpy_ok_gen:
  forall sz al,
  extcall_properties_gen (ec_to_gen (extcall_memcpy_sem sz al))
                         (mksignature (Tptr :: Tptr :: nil) Tvoid cc_default).
Proof.
  ii. eapply ec_implies_ec_gen. eapply extcall_memcpy_ok.
Qed.
...
Lemma extcall_annot_ok_gen:
  forall text targs,
  extcall_properties_gen (ec_to_gen (extcall_annot_sem text targs))
                         (mksignature targs Tvoid cc_default).
Proof.
  ii. eapply ec_implies_ec_gen. eapply extcall_annot_ok.
Qed.
...
Lemma extcall_annot_val_ok_gen:
  forall text targ,
  extcall_properties_gen (ec_to_gen (extcall_annot_val_sem text targ))
                         (mksignature (targ :: nil) targ cc_default).
Proof.
  ii. eapply ec_implies_ec_gen. eapply extcall_annot_val_ok.
Qed.
...
Lemma extcall_debug_ok_gen:
  forall targs,
  extcall_properties_gen (ec_to_gen extcall_debug_sem)
                         (mksignature targs Tvoid cc_default).
Proof.
  ii. eapply ec_implies_ec_gen. eapply extcall_debug_ok.
Qed.
...
Lemma known_builtin_ok_gen: forall bf,
  extcall_properties_gen (ec_to_gen (known_builtin_sem bf)) (builtin_function_sig bf).
Proof.
  ii. eapply ec_implies_ec_gen. eapply known_builtin_ok.
Qed.
...
Lemma capture_extends_backward
  ...
  (exists m2,
    ...
    /\ <<PRIV: Mem.unchanged_on (loc_out_of_bounds m1) m1' m2'>>).
Proof.
  exploit Mem.capture_extends_backward; eauto. i. des.
  esplits; eauto.
...
Proof.
  exploit Mem.capture_extends_backward_progress; eauto.
Qed.
...
Lemma capture_injects_backward
  ...
  (exists f' m2,
    ...
    /\ <<INJSEP: inject_separated f f' m1 m1'>>).
Proof.
  exploit Mem.capture_inject_backward; try eapply FB; eauto.
  i. des.
  esplits; eauto.
  - inversion CAPSRC.
    econs.
...
Lemma capture_injects_backward_progress
  ...
  \/ <<OOM: Mem.capture_oom m1' b'>>.
Proof.
  exploit Mem.capture_injects_backward_progress; eauto.
Qed.

Inductive extcall_capture_sem (ge: Senv.t):
              list val -> mem -> trace -> val -> mem -> Prop :=
  | extcall_capture_sem_pointer
      b m m' addr ofs v
      (CAPTURE: Mem.capture m b addr m')
      (RET: v = if Archi.ptr64
                then Vlong (Int64.repr (addr + (Ptrofs.unsigned ofs)))
    ...
    : 
      extcall_capture_sem ge (Vptr b ofs :: nil) m E0 v m'
  | extcall_capture_sem_long
      n m
      (BIT: Archi.ptr64 = true)
    :
      extcall_capture_sem ge (Vlong n :: nil) m E0 (Vlong n) m
  | extcall_capture_sem_int
      n m
...
Definition capture_args_wf m (vl: list val) : Prop :=
  match vl with
  | nil => False
  | hd::tl => match tl with
              | nil => match hd with
                       | Vint n => if Archi.ptr64 then False else True
                       | Vlong n => if Archi.ptr64 then True else False
                       | Vptr b ofs => if Mem.is_valid m b then True else False
                       | _ => False
                       end
              | hd'::tl' => False
              end
...
(* nb(oom) -> ~ extcall_capture_sem_lift ge vl m S -> it means no S *)
(* angelic stuck -> ub / demonic stuck -> nb *)
Inductive extcall_capture_sem_gen (ge: Senv.t):
              list val -> mem -> (trace -> val -> mem -> Prop) -> Prop :=
  | extcall_capture_sem_gen_success
      varg m t v m' (S: trace -> val -> mem -> Prop)
      (CAPTURE: extcall_capture_sem ge (varg :: nil) m t v m')
      (IN: S t v m')
      (SINGLE: forall t1 v1 m1, S t1 v1 m1 -> t1 = t /\ v1 = v /\ m1 = m')
      :
      extcall_capture_sem_gen ge (varg :: nil) m S
  | extcall_capture_sem_gen_ub
      vl m (S: trace -> val -> mem -> Prop)
      (ILLFORM: ~ capture_args_wf m vl)
      (EMPTY: forall t v m', ~ S t v m')
      :
      extcall_capture_sem_gen ge vl m S
.

(* Inductive extcall_capture_sem_gen (ge: Senv.t): *)
(*               list val -> mem -> (trace -> val -> mem -> Prop) -> Prop := *)
(*   | extcall_capture_sem_gen_intro *)
(*       vl m t v m' (S: trace -> val -> mem -> Prop) *)
(*       (CAPTURE: extcall_capture_sem ge vl m t v m') *)
(*       (IN: S t v m') *)
(*       (SINGLE: forall t1 v1 m1, S t1 v1 m1 -> t1 = t /\ v1 = v /\ m1 = m') *)
(*     : *)
(*     extcall_capture_sem_gen ge vl m S *)
(*   | extcall_capture_sem_gen_fail *)
(*       vl m *)
(*       (CAPFAIL: forall t v, ~ extcall_capture_sem ge vl m t v m) *)
(*     : *)
(*     extcall_capture_sem_gen ge vl m (fun t v m => False) *)
(* . *)

Lemma extcall_capture_ok:
  extcall_properties_gen extcall_capture_sem_gen
                         (mksignature (Tptr :: nil) (if Archi.ptr64 then Tlong else Tint) cc_default).
Proof.
  econs; i.
(* well typed *)
- inv DSTEP; cycle 1.
  { eapply EMPTY in ASTEP. clarify. }
  inv CAPTURE; unfold proj_sig_res; ss.
  + eapply SINGLE in ASTEP; eauto. des. des_ifs.
  + eapply SINGLE in ASTEP; eauto. des. des_ifs.
(* symbols *)
- inv DSTEPSRC; cycle 1.
  { exists (fun t v m => False). esplits.
    - econs 2; eauto.
    - ii. split; clarify. i. eapply EMPTY in H. clarify. }
  inv CAPTURE. 
  + exists S. econs; eauto. econs; eauto. econs; eauto.
  + exists S. econs; eauto. econs; eauto. econs 2; eauto.
  + exists S. econs; eauto. econs; eauto. econs 3; eauto.
(* valid blocks *)
- inv DSTEP1; cycle 1.
  { eapply EMPTY in ASTEP1. clarify. }
  inv CAPTURE; eauto.
  + eapply SINGLE in ASTEP1. i. des; subst.
    inv CAPTURE0. unfold Mem.valid_block in *. eauto.
    rewrite <- NEXTBLOCK. eauto.
  + eapply SINGLE in ASTEP1. i. des; subst. eauto.
  + eapply SINGLE in ASTEP1. i. des; subst. eauto.
(* max perms *)
- inv DSTEP; cycle 1.
  { eapply EMPTY in ASTEP. clarify. }
  inv CAPTURE. 
  + eapply SINGLE in ASTEP. des; subst. 
    inv CAPTURE0. unfold Mem.perm, Mem.perm_order' in *.
    rewrite ACCESS. eauto.
  + eapply SINGLE in ASTEP; des; subst. eauto.
  + eapply SINGLE in ASTEP; des; subst. eauto.
(* readonly *)
- inv DSTEP; cycle 1. 
  { eapply EMPTY in ASTEP. clarify. }
  ...
  + eapply SINGLE in ASTEP. des; subst. eauto.
(* mem extends *)
- inv DCALLTGT; cycle 1.
  (* SRC UB *)
  + assert (SRCILLFORM: ~ capture_args_wf m1 vargs).
    { ii. eapply ILLFORM. clear ILLFORM. 
      unfold capture_args_wf in *. des_ifs; inv ARGS; inv H3; inv H5.
      erewrite <- Mem.valid_block_iff_is_valid in Heq0.
      erewrite Mem.valid_block_extends in Heq0; eauto.
      erewrite Mem.valid_block_iff_is_valid in Heq0. clarify. }
    esplits.
    { econs 2; eauto. }
    ii. eapply EMPTY in ACALLSRC. clarify.
  + inv CAPTURE; cycle 1.
    { inv ARGS. inv H3; inv H2.
      - exists (fun t v m => (t = E0 /\ (v = Vlong n) /\ m = m1)). esplits.
        { econs; ss. econs; eauto. }
        i. des; subst. esplits; eauto. eapply Mem.unchanged_on_refl.
      - exists (fun t v m => False). esplits.
        { econs 2; ss. }
        ii. clarify. }
    { inv ARGS. inv H3; inv H2.
    ...
    { esplits.
      - econs 2; eauto.
        instantiate (1:= fun t v m => extcall_capture_sem ge [Vundef] m1 t v m).
        ii. inv H.
      - ii. ss. inv ACALLSRC. }
    esplits; ii.
    { econs.
      - econs; eauto.
      - instantiate (1:= fun t v m => (t = E0) /\ 
                                     (v = (if Archi.ptr64 
                                           then Vlong (Int64.repr (addr + Ptrofs.unsigned ofs))
                                           else Vint (Int.repr (addr + Ptrofs.unsigned ofs)))) /\
                                     (m = m2)).
...
- inv DCALLTGT; cycle 1.
  + assert (SRCILLFORM: ~ capture_args_wf m1 vargs).
    { ii. eapply ILLFORM. clear ILLFORM. 
      unfold capture_args_wf in *. des_ifs; inv ARGS; inv H3; inv H5.
      erewrite <- Mem.valid_block_iff_is_valid in Heq0.
      exploit Mem.mi_mappedblocks; eauto. i.
      erewrite Mem.valid_block_iff_is_valid in H0. clarify. }
    esplits.
    { econs 2; eauto. }
    ii. eapply EMPTY in ACALLSRC. clarify.
  + inv CAPTURE; cycle 1.
    ...
    { inv ARGS. inv H3; inv H2.
      ...
      - exists (fun t v m => False). esplits.
        { econs 2; ss. }
        ii. clarify. }
    inv ARGS. inv H3. dup H2. inv H2.
    2:{ exists (fun t v m => extcall_capture_sem ge1 [Vundef] m1 t v m). esplits.
        - econs 2; eauto. ii. inv H.
        - ii. ss. inv ACALLSRC. }
    replace addr with ((addr + delta) - delta) in CAPTURE0 by lia.
    exploit capture_injects_backward; try eapply CAPTURE0; eauto. i. des.
    exists (fun t v m => (t = E0) /\ 
                   (v = (if Archi.ptr64 
                         then Vlong (Int64.repr (addr + delta + Ptrofs.unsigned ofs1))
                         else Vint (Int.repr (addr + delta + Ptrofs.unsigned ofs1)))) /\
                   (m = m2)).
    esplits; ss.
    { econs; eauto. des_ifs. econs; eauto. }
    i. des. subst. des_ifs. esplits; eauto.
    replace (Int64.repr (addr + delta + Ptrofs.unsigned ofs1)) with (Int64.repr (addr + Ptrofs.unsigned (Ptrofs.add ofs1 (Ptrofs.repr delta)))); eauto.
    ...
    erewrite Ptrofs.modulus_eq64; eauto. rewrite (Zplus_mod_idemp_r delta (Ptrofs.unsigned ofs1)).
    rewrite Zplus_mod_idemp_r. f_equal. lia.
Qed.  
...
Parameter external_functions_sem: String.string -> signature -> extcall_sem_gen.

Axiom external_functions_properties:
  forall id sg, extcall_properties_gen (external_functions_sem id sg) sg.

(** We treat inline assembly similarly. *)

Parameter inline_assembly_sem: String.string -> signature -> extcall_sem_gen.

Axiom inline_assembly_properties:
  forall id sg, extcall_properties_gen (inline_assembly_sem id sg) sg.

(** ** Combined semantics of external calls *)

Definition builtin_or_external_sem name sg :=
  match lookup_builtin_function name sg with
  | Some bf => (ec_to_gen (known_builtin_sem bf))
  | None => external_functions_sem name sg
  end.

Lemma builtin_or_external_sem_ok: forall name sg,
  extcall_properties_gen (builtin_or_external_sem name sg) sg.
Proof.
...
Definition external_call (ef: external_function): extcall_sem_gen :=
  match ef with
  ...
  | EF_vload chunk       => (volatile_load_sem_gen chunk)
  | EF_vstore chunk      => (ec_to_gen (volatile_store_sem chunk))
  | EF_malloc            => (ec_to_gen extcall_malloc_sem)
  | EF_free              => (ec_to_gen extcall_free_sem)
  | EF_memcpy sz al      => (ec_to_gen (extcall_memcpy_sem sz al))
  | EF_annot kind txt targs   => (ec_to_gen (extcall_annot_sem txt targs))
  | EF_annot_val kind txt targ => (ec_to_gen (extcall_annot_val_sem txt targ))
  | EF_inline_asm txt sg clb => inline_assembly_sem txt sg
  | EF_debug kind txt targs => (ec_to_gen extcall_debug_sem)
  | EF_capture => extcall_capture_sem_gen
  end.

Theorem external_call_spec:
  forall ef,
  extcall_properties_gen (external_call ef) (ef_sig ef).
Proof.
  ...
  apply volatile_store_ok_gen.
  apply extcall_malloc_ok_gen.
  apply extcall_free_ok_gen.
  apply extcall_memcpy_ok_gen.
  apply extcall_annot_ok_gen.
  apply extcall_annot_val_ok_gen.
  apply inline_assembly_properties.
  apply extcall_debug_ok_gen.
  apply extcall_capture_ok.
Qed.

Definition external_call_well_typed_gen ef := ec_well_typed_gen (external_call_spec ef).
Definition external_call_symbols_preserved ef := ec_symbols_preserved_gen (external_call_spec ef).
Definition external_call_valid_block ef := ec_valid_block_gen (external_call_spec ef).
Definition external_call_max_perm ef := ec_max_perm_gen (external_call_spec ef).
Definition external_call_readonly ef := ec_readonly_gen (external_call_spec ef).
Definition external_call_mem_extends_gen ef := ec_mem_extends_gen (external_call_spec ef).
Definition external_call_mem_inject_general ef := ec_mem_inject_gen (external_call_spec ef).
(* Definition external_call_trace_length ef := ec_lifted_trace_length (external_call_spec ef). *)
(* Definition external_call_receptive ef := ec_lifted_receptive (external_call_spec ef). *)
...
.
(* TODO: fix this proofs. *)
Lemma internal_singleton
  ef
  (INT: ~ is_external_ef ef)
  se vl m S
  (CALL: external_call ef se vl m S)
  :
  <<SINGLETON: extcall_singleton S>>.
Proof.
  destruct ef; ss; des_ifs.
  - unfold builtin_or_external_sem in CALL. des_ifs.
    eapply ec_to_gen_singleton; eauto.
...
Lemma external_call_mem_extends
  ...
  (ARGS: Val.lessdef_list vargs vargs')    
  S_src
  (DSRC: external_call ef ge vargs m1 S_src)
  tr vres m2
  (ASRC: S_src tr vres m2)
  :
  exists S_tgt, exists vres', exists m2',
    <<DTGT: external_call ef ge vargs' m1' S_tgt>>
  /\ <<ATGT: S_tgt tr vres' m2'>>
  /\ <<RETV: Val.lessdef vres vres'>>
  /\ <<MEM: Mem.extends m2 m2'>>
  /\ <<PRIV: Mem.unchanged_on (loc_out_of_bounds m1) m1' m2'>>
...
Proof.
  destruct ef; ss; des_ifs.
  - unfold builtin_or_external_sem in *. des_ifs.
    eapply ec_to_gen_mem_extends; eauto.
    eapply known_builtin_ok.
  - unfold builtin_or_external_sem in *. des_ifs.
    eapply ec_to_gen_mem_extends; eauto.
    eapply known_builtin_ok.
...
Lemma external_call_mem_inject
  ...
  (ARGS: Val.inject_list f vargs vargs')
  S_src
  (DSRC: external_call ef ge1 vargs m1 S_src)
  tr vres m2
  (ASRC: S_src tr vres m2)
  :
  exists f', exists S_tgt, exists vres', exists m2',
    <<DTGT: external_call ef ge2 vargs' m1' S_tgt>>
  /\ <<ATGT: S_tgt tr vres' m2'>>
  /\ <<RETV: Val.inject f' vres vres'>>
  /\ <<MEM: Mem.inject f' m2 m2'>>
  /\ <<MEMPRIVSRC: Mem.unchanged_on (loc_unmapped f) m1 m2>>
...
Proof.
  destruct ef; ss; des_ifs.
  - unfold builtin_or_external_sem in *. des_ifs.
    eapply ec_to_gen_mem_inject; eauto.
    eapply known_builtin_ok.
  - unfold builtin_or_external_sem in *. des_ifs.
    eapply ec_to_gen_mem_inject; eauto.
    eapply known_builtin_ok.
...
Lemma external_call_well_typed:
  forall ef ge vargs m1 S t vres m2,
  external_call ef ge vargs m1 S -> S t vres m2 ->
...
Lemma external_call_mem_inject_gen
  ef F V (ge: Genv.t F V) vargs' m1' f m1 vargs
  (GENV: meminj_preserves_globals ge f)
  (INJ: Mem.inject f m1 m1')
  (ARGS: Val.inject_list f vargs vargs')
  (S_tgt: trace -> val -> mem -> Prop)
  (DCALLTGT: external_call ef ge vargs' m1' S_tgt)
  :
  (exists S_src (DCALLSRC: external_call ef ge vargs m1 S_src),
    (forall t vres m2 (ACALLSRC: S_src t vres m2),
      (exists vres' m2' (ACALLTGT: S_tgt t vres' m2'),
        exists f',
        <<RETV: Val.inject f' vres vres'>>
...
Proof.
  destruct GENV as (A & B & C). exploit external_call_mem_inject_general; eauto.
  repeat split; intros.
  + simpl in H0. exploit A; eauto. intros EQ; rewrite EQ in H; inv H. auto.
  + simpl in H0. exploit A; eauto. intros EQ; rewrite EQ in H; inv H. auto.
  + simpl in H0. exists b1; split; eauto.
...
Inductive eval_builtin_arg: builtin_arg A -> val -> Prop :=
  ...
  | eval_BA_loadstack_fail: forall chunk ofs,
      Mem.loadv chunk m (Val.offset_ptr sp ofs) = None ->
      eval_builtin_arg (BA_loadstack chunk ofs) Vundef                  
  | eval_BA_addrstack: forall ofs,
      eval_builtin_arg (BA_addrstack ofs) (Val.offset_ptr sp ofs)
  | eval_BA_loadglobal: forall chunk id ofs v,
      Mem.loadv chunk m (Senv.symbol_address ge id ofs) = Some v ->
      eval_builtin_arg (BA_loadglobal chunk id ofs) v
  | eval_BA_loadglobal_fail: forall chunk id ofs,
      Mem.loadv chunk m (Senv.symbol_address ge id ofs) = None ->
      eval_builtin_arg (BA_loadglobal chunk id ofs) Vundef
...
Proof.
  ...
  induction 1; eauto with barg. rewrite <- EQ in H; eauto with barg.
  eapply eval_BA_loadglobal_fail.
  { rewrite EQ. eauto. }
  rewrite <- EQ; eauto with barg.
Qed.
...
- destruct (Mem.loadv chunk m2 (Val.offset_ptr sp ofs)) eqn:TL.
+ exists v; eauto with barg.
+ exists Vundef. eauto with barg.
- econstructor; eauto with barg.
- exploit Mem.loadv_extends; eauto. intros (v' & P & Q). exists v'; eauto with barg.
- destruct (Mem.loadv chunk m2 (Senv.symbol_address ge id ofs)) eqn:TL.
+ exists v; eauto with barg.
+ exists Vundef; eauto with barg.
- econstructor; eauto with barg.

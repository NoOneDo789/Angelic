...
(*   (F V: Type) ef (ge: Genv.t F V) vargs1 m1 S2 m2 j P vargs2 *)
(*   (DCALLTGT: external_call ef ge vargs2 m2 S2) *)
(*   (SEP: m2 |= minjection j m1 ** globalenv_inject ge j ** P) *)
(*   (ARGS: Val.inject_list j vargs1 vargs2) *)
(*   : *)
(*   (exists S1 (DCALLSRC: external_call ef ge vargs1 m1 S1), *)
(*      (forall t vres1 m1' (ACALLSRC: S1 t vres1 m1'), *)
(*         (exists vres2 m2' (ACALLTGT: S2 t vres2 m2'), *)
(*          exists j', *)
(*            <<RETV: Val.inject j' vres1 vres2>> *)
(*          /\ <<SEP': m2' |= minjection j' m1' ** globalenv_inject ge j' ** P>> *)
...
(*   eapply globalenv_inject_preserves_globals. eapply sep_pick1; eauto. i. des. *)
(*   esplits; eauto. i. exploit H; eauto. clear H. *)
(*   intros (vres2 & m2' & ACALLTGT & j' & RES & INJ' & UNCH1 & UNCH2 & INCR & ISEP). *)
(*   assert (MAXPERMS: forall b ofs p, *)
(*             Mem.valid_block m1 b -> Mem.perm m1' b ofs Max p -> Mem.perm m1 b ofs Max p). *)
(*   { intros. eapply external_call_max_perm; eauto. } *)
(*   exists vres2, m2'. split; [eauto | exists j']; intuition auto. *)
(*   split; [|split]. *)
(* - exact INJ'. *)
(* - apply (m_invar _ m2). *)

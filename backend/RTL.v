...
Require Import sflib Simulation Classical.
...
Inductive instruction: Type :=
  ...
  (* | Iitop: list reg -> reg -> node -> instruction *)
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Iop:
      forall s f sp pc rs m op args res pc' v (NITOP: op <> Oitop),
      (fn_code f)!pc = Some(Iop op args res pc') ->
      eval_operation ge sp op rs##args m = Some v ->
      step (State s f sp pc rs m)
        E0 (State s f sp pc' (rs#res <- v) m)
  | exec_Iop_itop:
      forall s f sp pc rs m op args res pc' v (ITOP: op = Oitop),
      (fn_code f)!pc = Some(Iop op args res pc') ->
      In v (int2ptr_operation (rs##args) m) -> (* for angelic choice *)
      (* eval_operation ge sp op rs##args m = Some v -> *)
      step (State s f sp pc rs m)
        E0 (State s f sp pc' (rs#res <- v) m)
  | exec_Iload:
  ...
  | exec_Ibuiltin:
      forall s f sp pc rs m ef args res pc' vargs S t vres m' (SINGLE: extcall_singleton S),
      (fn_code f)!pc = Some(Ibuiltin ef args res pc') ->
      eval_builtin_args ge (fun r => rs#r) sp m args vargs ->
      external_call ef ge vargs m S -> S t vres m' ->
      step (State s f sp pc rs m)
  ...
  | exec_function_external:
      forall s ef args S res t m m' (SINGLE: extcall_singleton S),
      external_call ef ge args m S -> S t res m' ->
...
Inductive external_step: state -> (trace -> state -> Prop) -> Prop :=
  | external_builtin
      s f sp pc rs m ef args res pc' vargs
      (S_ext: trace -> val -> mem -> Prop)
      (S_step: trace -> state -> Prop)
      (PC: (fn_code f)!pc = Some(Ibuiltin ef args res pc'))
      (EVALARGS: eval_builtin_args ge (fun r => rs#r) sp m args vargs)
      (EXTCALL: external_call ef ge vargs m S_ext)
      (IN: forall t vres m', S_ext t vres m' -> 
                             S_step t (State s f sp pc' (regmap_setres res vres rs) m'))
      :
      external_step (State s f sp pc rs m) S_step
  | external_external_call
      s ef args m
      (S_ext: trace -> val -> mem -> Prop)
      (S_step: trace -> state -> Prop)
      (EXTCALL: external_call ef ge args m S_ext)
      (IN: forall t res m', S_ext t res m' ->
                            S_step t (Returnstate s res m'))
      :
      external_step (Callstate s (External ef) args m) S_step
.

Lemma exec_Iop':
  forall s f sp pc rs m op args res pc' rs' v (NITOP: op <> Oitop),
  (fn_code f)!pc = Some(Iop op args res pc') ->
  eval_operation ge sp op rs##args m = Some v ->
  rs' = (rs#res <- v) ->
  step (State s f sp pc rs m)
...
Definition final_state_fun (st: state) : option int :=
  match st with
  | Returnstate nil (Vint r) m => Some r
  | _ => None
  end.
...
Definition is_external (ge:genv) (s:state) : Prop :=
  ...
  | Callstate stk fd vargs m =>
    match fd with
    | External ef => (* is_external_ef ef *) True
    | _ => False
    end
  | State cs f sp pc rs m =>
    match (fn_code f)!pc with
    | Some (Ibuiltin ef args res pc') => (* is_external_ef ef *) True
    | _ => False
    end
  | _ => False
  end.
...
Definition semantics (p: program) :=
  Semantics step (initial_state p) final_state (* is_external *) (Genv.globalenv p).

Program Definition gsemantics (p: program) :=
  GSemantics_gen (step_gen step external_step is_external (fun ge => final_state_fun)) (initial_state p) final_state_fun (Genv.globalenv p) (Genv.globalenv p) _.
Next Obligation.
  ii. inv H0; clarify. destruct s; ss.
Qed.

...
Lemma eval_builtin_arg_inject_backward:
  forall rs sp m j rs' sp' m' a v',
  eval_builtin_arg tge (fun r => rs'#r) (Vptr sp' Ptrofs.zero) m' a v' ->
  j sp = Some(sp', 0) ->
  meminj_preserves_globals j ->
...
- econstructor; eauto with barg.
- econstructor; eauto with barg.
- simpl in H. rewrite Ptrofs.add_zero_l in H.
  replace (Ptrofs.unsigned ofs) with ((Ptrofs.unsigned ofs) + 0) in H by lia.
  exploit Mem.load_inject_backward; eauto. i. des.
  + esplits; eauto. econs; eauto. ss. rewrite Ptrofs.add_zero_l; auto.
  + exists Vundef. esplits; eauto. eapply eval_BA_loadstack_fail.
    ss. rewrite Ptrofs.add_zero_l; auto.
- destruct (Mem.loadv chunk m (Val.offset_ptr (Vptr sp Ptrofs.zero) ofs)) eqn:TL.
  + exploit Mem.load_inject; try eapply TL; eauto. i. des.
    ss. rewrite Z.add_0_r in H0. clarify.
  + esplits; eauto. eapply eval_BA_loadstack_fail; eauto.
- econstructor; split; eauto with barg. simpl. econstructor; eauto. rewrite Ptrofs.add_zero; auto.
- assert (Val.inject j (Senv.symbol_address ge id ofs) (Senv.symbol_address tge id ofs)).
  { unfold Senv.symbol_address; simpl; unfold Genv.symbol_address.
    destruct (Genv.find_symbol ge id) as [b|] eqn:FS; auto.
    exploit symbols_inject_2; eauto. intros (b' & A & B). rewrite A.
    econstructor; eauto. rewrite Ptrofs.add_zero; auto. }
  exploit Mem.loadv_inject_backward; eauto. i. des.
  + esplits; eauto. eauto with barg.
...
- assert (Val.inject j (Senv.symbol_address ge id ofs) (Senv.symbol_address tge id ofs)).
  { unfold Senv.symbol_address; simpl; unfold Genv.symbol_address.
    ...
    exploit symbols_inject_2; eauto. intros (b' & A & B). rewrite A.
    econstructor; eauto. rewrite Ptrofs.add_zero; auto. }
  destruct (Mem.loadv chunk m (Senv.symbol_address ge id ofs)) eqn:TL.
  + exploit Mem.loadv_inject; try eapply TL; eauto. i. des; clarify.
  + esplits; eauto. eapply eval_BA_loadglobal_fail; eauto.
- econstructor; split; eauto with barg.
  unfold Senv.symbol_address; simpl; unfold Genv.symbol_address.
  destruct (Genv.find_symbol ge id) as [b|] eqn:FS; auto.
  exploit symbols_inject_2; eauto. intros (b' & A & B). rewrite A.
...
Lemma eval_builtin_args_inject_backward:
...
- simpl in H5.
  exploit eval_builtin_arg_inject_backward; eauto using in_or_app. intros (v1' & A & B).
  destruct IHlist_forall2 as (vl' & C & D); eauto using in_or_app.
  exists (v1' :: vl'); split; constructor; auto.
Qed.

Theorem step_simulation:
  forall S1 t S2 (INT: ~ is_external ge S1), step ge S1 t S2 ->
  forall S1' (MS: match_states S1 S1'),
  exists S2', step tge S1' t S2' /\ match_states S2 S2'.
Proof.
  induction 2; intros; inv MS.
...
- (* itop *)
  exploit int2ptr_operation_inject; try eapply H0; eauto. apply regs_inject; auto. eauto.
  intros (tv & B & C). des.
  econstructor; split. eapply exec_Iop_itop; eauto.
  econstructor; eauto. apply set_reg_inject; auto.
...
Lemma transf_initial_states:
  forall S1, RTL.initial_state p S1 -> exists S2, RTL.initial_state tp S2 /\ match_states S1 S2.
Proof.
...
Lemma transf_final_states:
  forall S1 S2 r,
  match_states S1 S2 -> RTL.final_state S1 r -> RTL.final_state S2 r.
Proof.
...
Lemma transf_final_states':
  forall st1 st2 r,
  match_states st1 st2 -> final_state_fun st1 = Some r -> final_state_fun st2 = Some r.
Proof.
  i. inv H; ss; des_ifs; inv RESINJ; eauto. inv STACKS.
Qed.
...
Lemma match_states_gsim
  ...
  :
    gsim (RTL.gsemantics p) (RTL.gsemantics tp) false false None (inl st_src0) (inl st_tgt0).
Proof.
  generalize dependent st_src0. generalize dependent st_tgt0.
  pcofix CIH. i. pfold.
  destruct (final_state (RTL.gsemantics tp) st_tgt0) eqn:FINAL.
  { destruct (final_state (RTL.gsemantics p) st_src0) eqn:SRC.
    2:{ inv MATCH; ss. des_ifs_safe. des_ifs; inv RESINJ; try by inv STACKS.
        eapply gsim_dstep_src; eauto. econs; ss.
        eapply gsim_astep_src; eauto. ii. ss. des. subst. inv ASRC. }
    assert (i0 = i).
    { inv MATCH; ss. des_ifs; inv RESINJ. eauto. }
    subst. eapply gsim_final; eauto. }
  ...
  - eapply gsim_dstep_tgt; eauto. i.
    ...
    { instantiate (1:=(fun t st_src1 => RTL.step ge st_src0 t st_src1 /\ t = E0)).
      econs; eauto.
      destruct (final_state_fun st_src0) eqn:FINALSRC; eauto.
      exploit transf_final_states'; eauto. i. clarify. ss. clarify. }
    eapply gsim_astep_src; eauto. i. ss. des. subst.
    (* make lemma *)
    assert (EXTT: ~ is_external ge st_tgt0).
    { inv MATCH; ss; des_ifs_safe. }
    exploit step_simulation; try eapply H; eauto. i. des.
    inv DTGT; clarify.
  ...
  - eapply gsim_dstep_tgt; eauto. i.
    ...
    + unfold RTL.is_external in H. destruct st_src0; ss; inversion MATCH; subst.
      exploit eval_builtin_args_inject_backward; eauto.
      { eapply match_stacks_preserves_globals; eauto. }
      { i. eapply KEPT. r. esplits; eauto. }
      intros (svargs & SARGS & ARGSINJ).
      exploit external_call_mem_inject_general; eauto.
      { eapply globals_symbols_inject; auto.
        eapply match_stacks_preserves_globals; eauto. }
      i. des.
      (* exploit match_stacks_preserves_globals; eauto. i. *)
      set (S_src1:= (fun tr st => match st with
                               | State s1 f1 sp pc1 rs1 m1 =>
                                   s1 = stack /\ f1 = f /\ sp = (Vptr sp1 Ptrofs.zero) /\ pc1 = pc' /\
                                     exists v, S_src tr v m1 /\ rs1 = (regmap_setres res v rs0)
                                     (* (match res with *)
      ...
      eapply gsim_dstep_src; eauto.
      { econs 2; ss. econs; eauto. i. instantiate (1:=S_src1). ss. esplits; eauto. }
      eapply gsim_astep_src; eauto. i.
      subst S_src1. ss. des_ifs. des; clarify.
      ...
      apply match_stacks_incr with j; auto.
      intros. exploit INJSEP; eauto. intros [U V].
      assert (Mem.valid_block m0 sp1) by (eapply Mem.valid_block_inject_1; eauto).
      assert (Mem.valid_block m tsp) by (eapply Mem.valid_block_inject_2; eauto).
      unfold Mem.valid_block in *; extlia.
      apply set_res_inject; auto. apply regset_inject_incr with j; auto.
    + unfold RTL.is_external in H. destruct st_src0; ss; inversion MATCH; subst.
      exploit external_call_mem_inject_general; eauto.
      ...
      i. des.
      set (S_src1:= (fun tr st => match st with
                               | Returnstate s1 vres1 m1 => s1 = stack /\ S_src tr vres1 m1
                               | _ => False
                               end)).
      eapply gsim_dstep_src; eauto.
      { econs 2; ss. econs; eauto. i. instantiate (1:=S_src1). ss. }
      eapply gsim_astep_src; eauto. i.
      subst S_src1. ss. des_ifs. des; clarify.
      exploit H0; eauto. i. des.
      exploit IN; eauto. i.
      ...
      apply match_stacks_bound with (Mem.nextblock m0) (Mem.nextblock m).
      apply match_stacks_incr with j; auto.
...
Lemma deterministic_initial_state:
  forall st0 st1, RTL.initial_state tp st0 -> RTL.initial_state tp st1 -> st0 = st1.
Proof. ii. inv H; inv H0; ss. clarify. subst ge0 ge1. clarify. Qed.

Lemma transf_program_correct_1:
  general_simulation (RTL.gsemantics p) (RTL.gsemantics tp).
Proof.
  econs. econs.
  destruct (classic (exists st_src, initial_state (RTL.gsemantics p) st_src)).
  2:{ econs; eauto. }
  des. econs 2; eauto.
...
Theorem transf_program_correct:
  forall p tp, match_prog p tp -> general_simulation (RTL.gsemantics p) (RTL.gsemantics tp).
Proof.

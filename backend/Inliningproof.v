...
(*   split. eapply star_right. eauto. eapply exec_Iop; eauto. clarify. traceEq. *)
...
(*   forall S1 t S2 (INT: ~ is_external ge S1), *)
(*   step ge S1 t S2 -> *)
...
(*   left; econstructor; split. *)
(*   eapply plus_one. eapply exec_Iop; try eapply H9; eauto. *)
(*   { ii. destruct op; ss. } *)
...
(* - (* itop *) *)
(*   exploit tr_funbody_inv; eauto. intros TR; inv TR. *)
(*   exploit int2ptr_operation_inject; try eapply H0; eauto. *)
...
(*   (* fold (sop ctx op). intros [v' [A B]]. *) *)
(*   left; econstructor; split. *)
(*   eapply plus_one. eapply exec_Iop_itop; try eapply H9; eauto. *)
(*   (* erewrite eval_operation_preserved; eauto. *) *)
(*   (* exact symbols_preserved. *) *)
(*   econstructor; eauto. *)
(*   apply match_stacks_inside_set_reg; auto. *)
...
(* + (* with a result *) *)
(*   left; econstructor; split. *)
(*   eapply plus_one. eapply exec_Iop; eauto. ii. clarify. simpl. reflexivity. *)
...
(*   forall st1, RTL.initial_state prog st1 -> exists st2, RTL.initial_state tprog st2 /\ match_states st1 st2. *)
(* Proof. *)
(*   intros. inv H. *)
...
(* Proof. *)
(*   i. inv H; ss; des_ifs; inv VINJ; eauto. *)
(*   - exploit match_stacks_empty; eauto. intros EQ; subst. clarify. *)
(*   - exploit match_stacks_inside_empty; eauto. intros [A B]. congruence. *)
(* Qed. *)

(* (* Lemma transf_final_states': *) *)
(* (*   forall st1 st2 r, *) *)
(* (*   match_states st1 st2 -> RTL.final_state st2 r -> RTL.final_state st1 r. *) *)
(* (* Proof. *) *)
(* (*   intros. inv H0. inv H. inv MS; ss. *) *)
(* (*   exploit match_stacks_empty; eauto. intros EQ; subst. inv VINJ. constructor. *) *)
(* (*   exploit match_stacks_inside_empty; eauto. intros [A B]. congruence. *) *)
(* (* Qed. *) *)
...
(*   { destruct (final_state (RTL.gsemantics prog) st_src0) eqn:SRC. *)
(*     2:{ inv MATCH; ss. des_ifs_safe. des_ifs; try by (inv VINJ; inv MS). *)
(*         eapply gsim_dstep_src; eauto. econs; ss. *)
(*         eapply gsim_astep_src; eauto. ii. ss. des. subst. inv ASRC. } *)
(*     assert (i0 = i). *)
(*     { inv MATCH; ss. des_ifs; try by (inv VINJ; inv MS). } *)
...
(*       - inv FB. des_ifs. *)
(*         all: try (exploit H1; eauto; i; inv H6; ss; clarify). *)
(*         + exploit H1; eauto. i. inv H6; clarify. *)
(*           unfold inline_return in H7. des_ifs. *)
(*         + inv ASRC; clarify. *)
(*       - ss. unfold Errors.bind in FD. des_ifs. *)
(*       - rewrite NOP. ss. *)
(*       - unfold inline_return in Heq0. des_ifs. } *)
(*     exploit step_simulation; try eapply H; eauto. i. des. *)
(*     +  *)
(*     des. *)
(*     2:{ eapply gsim_dstep_tgt; eauto. *)
(*     des; cycle 1. *)
